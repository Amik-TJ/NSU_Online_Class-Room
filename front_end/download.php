...
$sql = "SELECT created_by, material FROM post WHERE class_id = :class_id and material <> 'NULL'";

// Prepare Statement
if ($stmt = $conn->prepare($sql)) {
    // Executing Query
    if ($stmt->execute(array(':class_id' => $class_id))) {
        $num = $stmt->rowCount();
        $items = array();
        if ($num > 0) {
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                $download_item = array(
                    'person_id' => $row['created_by'],
                    'material' => $row['material']
                );
                $success = 1;
                array_push($items, $download_item);
            }
        } else {
            $success = 0;
        ...
        $success = 0;
        echo "Problem on Executing Query";
    }
} else {
    echo "Problem on Preparing Query";
}
...
<div class="jumbotron jumbotron-fluid bg-info text-white text-center">
    <div class="container">
        <h1 class="display-1"><?php echo "Downloads" ?></h1>
...
<div class="container-fluid">
    <div class="row">
        ...
        <div class="col-md-6">
                ...
                echo '</thead>';
                echo '<tbody>';
                foreach ($items as $down) {
                        $f_name = $class_id."$".$down['person_id']."$".$down['material'];
                        $file_url = "download.php?file=".$f_name;
                        echo '<tr>';
                        echo '<th scope="row">';
                        echo $down['material'];
                        echo '</th>';
                        ...
                        echo '<button type="button" class="btn btn-primary ">Download</button>';
                        echo '</a>';
                        echo '</td>';
                        echo '</tr>';
                    }
                ...
                echo '</table>';
                    ...
                    echo '<h1 class="text-center">No Downloads Available</h1>';
                }
                ?>
